...
import { Calendar, QrCode, Plus, Star, ChevronRight, XCircle, Clock, Check } from "lucide-react";
...
export default function ClienteDashboard() {
  ...
  // Cita confirmada reciente (primer estado "confirmada" futura) — para banner especial
  const proximaConfirmada = citasProximas.find((c) => c.estado === "confirmada");
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Banner: Cita Confirmada — visible solo cuando hay cita confirmada */}
      {proximaConfirmada && (
        <div className="relative overflow-hidden rounded-3xl border border-teal-500/30 bg-gradient-to-br from-teal-500/10 via-[var(--card)] to-teal-600/5 shadow-lg shadow-teal-500/10">
          {/* Efectos decorativos */}
          <div className="absolute -top-12 -right-12 w-32 h-32 bg-teal-500/10 rounded-full blur-2xl"></div>
          <div className="absolute -bottom-8 -left-8 w-24 h-24 bg-teal-500/5 rounded-full blur-xl"></div>
          ...
          <div className="absolute top-4 left-4 flex items-center gap-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-teal-400">Cita Confirmada</p>
          </div>
          ...
          <div className="relative p-6 pt-12">
            ...
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                <h3 className="text-xl font-black text-[var(--white)] mb-1">{proximaConfirmada.servicio_nombre}</h3>
                <p className="text-sm text-[var(--muted)]">
                  {proximaConfirmada.barberia_nombre}
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-2xl font-black text-[var(--gold)]">{formatPrecio(proximaConfirmada.precio)}</p>
                <p className="text-[10px] text-[var(--muted)] uppercase tracking-wider">MXN</p>
              </div>
            </div>

            {/* Detalles: Barvero, Fecha, Hora, Duración */}
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-white/5 rounded-xl p-3 text-center">
                <p className="text-[10px] text-[var(--muted)] uppercase tracking-wider mb-1">Barbero</p>
                <p className="text-sm font-bold text-[var(--white)] truncate">{proximaConfirmada.barbero_nombre}</p>
              </div>
              <div className="bg-white/5 rounded-xl p-3 text-center">
                <p className="text-[10px] text-[var(--muted)] uppercase tracking-wider mb-1">Fecha</p>
                <p className="text-sm font-bold text-[var(--white)]">
                  {new Date(proximaConfirmada.fecha + "T12:00:00").toLocaleDateString("es-MX", { weekday: "short", day: "numeric", month: "short" })}
                </p>
              </div>
              <div className="bg-white/5 rounded-xl p-3 text-center">
                <p className="text-[10px] text-[var(--muted)] uppercase tracking-wider mb-1">Hora</p>
                <p className="text-sm font-bold text-[var(--white)]">{proximaConfirmada.hora}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 text-[var(--muted)]">
                ...
                <span>Duración aproximada: {proximaConfirmada.duracion_min} min</span>
              </div>
              <div className="flex items-center gap-1 bg-teal-500/10 text-teal-400 px-3 py-1 rounded-full">
                <Check size={12} />
                <span className="font-bold text-[10px] uppercase tracking-wider">Confirmada</span>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-teal-500/20">
              <p className="text-xs text-[var(--muted)] text-center">
                Te esperamos. Presenta este comprobante al llegar.
              </p>
            </div>
          </div>
        </div>
